...
export default function MobileApp() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.altKey && e.key === "l") {
        e.preventDefault();
        if (selectedId) {
          setItemModalHeadingId(selectedId);
  ...
  // When a heading is selected from mobile, immediately open the item modal
  const handleHeadingClick = (id) => {
    setSelectedId(id);
    // On mobile: tap heading to add item (double-click too slow)
    setItemModalHeadingId(id);
    setItemModalOpen(true);
  };

  return (
    <Layout>
      <Lhs headings={headings} selectedId={selectedId} onSelect={handleHeadingClick} onToggleItem={toggleItem} onDeleteItem={deleteItem} onEditItem={(itemId) => setEditItemKey(itemId)} onAddListToHeading={(id) => { setItemModalHeadingId(id); setItemModalOpen(true); }} />

      <Rhs onAddHeading={() => setHeadingModalOpen(true)} onAddList={() => { if (selectedId) { setItemModalHeadingId(selectedId); setItemModalOpen(true); } }} hasSelection={!!selectedId} />
